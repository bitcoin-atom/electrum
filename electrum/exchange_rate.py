...
CCY_PRECISIONS = {'BHD': 3, 'BIF': 0, 'BYR': 0, 'CLF': 4, 'CLP': 0,
                  ...
                  # Cryptocurrencies
                  'BTC': 8, 'BCA': 8, 'LTC': 8, 'XRP': 6, 'ETH': 18,
                  }
...
class ExchangeBase(Logger):
    ...
    async def get_json(self, site, get_string, cookies = None):
        # APIs must have https
        url = ''.join(['https://', site, get_string])
        network = Network.get_instance()
        proxy = network.proxy if network else None
        async with make_aiohttp_session(proxy, cookies=cookies) as session:
            async with session.get(url) as response:
                response.raise_for_status()
...
class YahooFinance(ExchangeBase):
    ...
    def __init__(self, on_quotes, on_history):
        ExchangeBase.__init__(self, on_quotes, on_history)
        self.crumb_cookies = None
        self.crumb_cache = None

    async def get_crumb(self):
        if self.crumb_cache:
            return [self.crumb_cookies, self.crumb_cache]
        # APIs must have https
        network = Network.get_instance()
        proxy = network.proxy if network else None
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36'}
        try:
            async with make_aiohttp_session(proxy, headers=headers) as session_1:
                async with session_1.get('https://fc.yahoo.com') as response_1:
                    response_1.raise_for_status()
                    # set content_type to None to disable checking MIME type
                    await response_1.json(content_type=None)
        except:
            pass
        self.crumb_cookies = response_1.cookies
        async with make_aiohttp_session(proxy, headers=headers, cookies=self.crumb_cookies) as session_2:
            async with session_2.get('https://query1.finance.yahoo.com/v1/test/getcrumb') as response_2:
                response_2.raise_for_status()
                # set content_type to None to disable checking MIME type
                self.crumb_cache = await response_2.json(loads=(lambda x: x), content_type='text/plain;charset=utf-8')
        return [self.crumb_cookies, self.crumb_cache]

    async def get_rates(self, ccy):
        cookies, crumb = await self.get_crumb()
        site = 'query1.finance.yahoo.com'
        get_string = f'/v10/finance/quoteSummary/BCA-USD?modules=price&crumb={crumb}'
        json = await self.get_json(site, get_string, cookies=cookies)
        try:
            regular_market_price_raw = json['quoteSummary']['result'][0]['price']['regularMarketPrice']['raw']
        except:
            return dict()
        return dict([('USD', to_decimal(regular_market_price_raw))])
